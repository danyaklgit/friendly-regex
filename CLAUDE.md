...
>
> **Last updated:** 2026-05-20 against branch `main`, head `b56f10a`.
...
- **Dual-mode pagination**: offset mode shows page totals; incremental mode is "+N more" with overfetch logic to account for client-side hidden tags (gotcha #2).
...
1. **Hidden TagSpecs survive tab navigation** (commit `cd2e253`). When the user hides definitions in the Transactions tab and switches tabs, the hidden state must persist through the full switch-and-back cycle. Implementation lives in `TransactionDataContext`. Do not reset hidden state on `TransactionsTab` remount.
2. **Footer counts subtract hidden, action buttons stay raw** (commits `1a912a3`, `1ad11d4`). Two different numbers in the same UI by design. Footer shows the operator-relevant count; action buttons show the underlying dataset count. When the user clicks `+50` with hidden tags active, the page must overfetch so 50 *visible* rows are added (gotcha #11 in section 8.2).
...
| Hidden tags / pagination                      | [src/context/TransactionDataContext.tsx](src/context/TransactionDataContext.tsx), [src/components/transactions/HiddenTagsPanel.tsx](src/components/transactions/HiddenTagsPanel.tsx) (in `transactions/` folder), [src/hooks/useTransactionData.ts](src/hooks/useTransactionData.ts)                                            |
...
3. `npm run dev`, open the app, and walk the golden path for Transactions:
   ...
   3. Hide a tag in a row -> confirm footer count subtracts hidden + button counts stay raw.
   4. Click `+50` pagination -> confirm visible rows grow by 50, not by 50 + hidden.
