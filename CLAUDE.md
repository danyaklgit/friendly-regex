...
(Removed 2026-06-17: `VITE_TEP_API_KEY` / the `x-apikey` header. That key was for integration services, not the Portal; the Portal authenticates with the per-user JWT + `TTPUserId`/`TTPTenantCode` bundle. Do not reintroduce it.)
...
- **TepHeaders**. The standard header bundle (`userId`, `tenantCode`, `languageCode`, `timeZone`, `requestId`) every TEP call carries (mapped to `TTPUserId`/`TTPTenantCode`/etc. headers; auth is the Bearer JWT). The `apiKey` field / `x-apikey` header was removed 2026-06-17 (integration-services key, not for the Portal). Constructed in ~18 call sites; refactor candidate `useTepHeaders()`.
...
2. **SUPERSEDED 2026-06-12 — hidden-tag exclusion is server-side via a DUAL QUERY, and all pagination/count numbers run in VISIBLE space.** The old convention (footer subtracts hidden, action buttons stay raw, commits `1a912a3`/`1ad11d4`) is gone. `replaceFromBeginningExcluding` in [src/context/TransactionDataContext.tsx](src/context/TransactionDataContext.tsx) fires TWO parallel queries: (A) visible set — active filters + TWO separate `NI` properties `{ColumnName:'OpsTagSpecDefinitionId',Operand:'NI'}` AND `{ColumnName:'OpsMultiTags.TagSpecDefinitionId',Operand:'NI'}` (pipe-joined ids; the ONLY payload that drops multi-tag-hidden rows — the single COMPOSITE column `OpsTagSpecDefinitionId|OpsMultiTags.TagSpecDefinitionId` ignored the array and leaked them, 230/250, do NOT revert). **The backend KEEPS untagged (NULL-tag) rows under `NI`, so query A alone IS the exact visible set and its `TransactionsCount` is the exact visible total (`totalShowing`)** — no merge, no sum. (HISTORY: an earlier design added a second `{OpsIsUntagged EQ True}` query on the belief `NI` dropped NULL rows — the "bd1267f trap". Verified live 2026-06-15 that it does NOT on this backend: the NI query returned total-minus-hidden INCLUDING untagged, so the extra half double-counted untagged rows in both the total (`5836+166=6002` vs a true `5836`) and the buffer. That half + the `mergeSortedRows` merge were removed.) (B) scope total — active filters only, `PageSize 1`, NO exclusion: its `TransactionsCount` is the no-exclusion scope total (the same number a plain "main" load produces) and is written into `totalTransactionsCount`. **"N hidden" = `totalTransactionsCount` (main-load total, refreshed by query B) − visibleTotal** — a pure difference of the fetch responses, with NO separate engine-level count call (the old `resolveUnfilteredTotal` is gone). Query B keeps the tally correct even when filters change while tags are hidden (the plain load doesn't run then, so `totalTransactionsCount` would otherwise be stale). [src/hooks/useVisibleRowsEngine.ts](src/hooks/useVisibleRowsEngine.ts) owns the target/refill/counts. `mergeSortedRows`/`compareBySorting` in [src/utils/visibleRows.ts](src/utils/visibleRows.ts) are now unused by the live path (kept + tested in case the union approach is ever needed again). The CSV **export** (`handleExport` in [src/components/transactions/TransactionsTab.tsx](src/components/transactions/TransactionsTab.tsx)) appends the SAME two `NI` filters when `hiddenDefIds` is non-empty, so the exported rows match the visible set (untagged kept). The two `NI` filters EXCLUDE mixed-tag rows entirely (a hidden multi-tag fails NI), so `displayAnalyzedData`'s hidden-strip in [src/components/transactions/TransactionsTab.tsx](src/components/transactions/TransactionsTab.tsx) is a no-op safety net in live mode (it matters only if the server ever keeps a mixed row). The client-side `isRowHidden` row-drop pass + `hiddenLoadedCount` run in SAMPLE mode only (live mode trusts the server). `targetVisible` is the incremental DISPLAY window (`visibleData` slices to it), never shrunk by a hide; **unhide / unhide-all reset it to PAGE_SIZE (50)** and refetch. It is NOT persisted: leaving the Transactions tab (unmount) and returning resets the view to the initial 50 regardless of prior +N / Show all (a ref carries it WITHIN a tab session so filter changes keep the window; only unmount drops it). The footer "loaded" count tracks the loaded BUFFER (`transactions.length`), not `filteredData.length` — the latter is built from the chunked `analyzeRow` pass and made Show all appear to load "in increments of 500" while rows were merely being analyzed. The virtualizer must re-sync on data change (gotcha #25). Design: [docs/superpowers/specs/2026-06-12-hidden-tags-refill-design.md](docs/superpowers/specs/2026-06-12-hidden-tags-refill-design.md).
...
28. **The transformation dropdown is backend-LOV-driven; local-only methods are HIDDEN.** [src/context/LovAttributesContext.tsx](src/context/LovAttributesContext.tsx) builds `transformationMethods` from the backend `ATTRIBUTE_TRANSFORMATON` LOV (note the typo, no second "I") whenever that list is non-empty; it only borrows `args`/`description`/`category` from the local [src/constants/transformations.ts](src/constants/transformations.ts) catalog by matching `item.Value === method.key`. It does NOT union the two, so a method that exists ONLY in the local catalog (e.g. `collapse_whitespace` / "Remove Extra Blanks") never appears in the picker. To SHIP a new transformation: the backend must add it to the `ATTRIBUTE_TRANSFORMATON` LOV (matching `Value` = local `key`) AND implement it server-side; the frontend mirrors the implementation in [src/utils/transformations.ts](src/utils/transformations.ts) (the `applyTransformation` switch) + a catalog entry, purely so the wizard PREVIEW works. `collapse_spaces` (added 2026-06-15) is the latest example. `applyTransformation` is the source of truth for preview behavior; keep it byte-compatible with the backend.
29. **Narrative cells preserve consecutive spaces; do not "normalize" them.** Bank narratives (`AdditionalInformation`, `Description1/2`, `TransactionDetails`) carry padding runs of spaces that operators split rules on, so the UI must render them verbatim. The table has TWO modes via the "Compact mode" toggle (`relaxedMode`, which **defaults to true** — `tep:relaxedMode`), and `CellContentWrapper` in [src/components/transactions/TransactionTable.tsx](src/components/transactions/TransactionTable.tsx) branches on it: expanded mode wraps narrative content in `line-clamp-3 whitespace-pre-wrap`, **compact mode wraps it in `whitespace-pre`** (single line; this is the default branch and was the one collapsing spaces — the `<td>` carries `whitespace-nowrap` and in compact mode `resolveColumnWidth` returns undefined, so without the explicit narrative `whitespace-pre` child the text inherited the td's space-collapsing `nowrap`). [ViewContextModal.tsx](src/components/transactions/ViewContextModal.tsx) cells use `whitespace-pre` (single-line, the inspector scrolls horizontally); [AttributeDisplay.tsx](src/components/transactions/AttributeDisplay.tsx) and the wizard extraction/transformation preview `<code>` blocks ([TransformationPreview.tsx](src/components/wizard/TransformationPreview.tsx), [AttributeEditor.tsx](src/components/wizard/AttributeEditor.tsx)) use `whitespace-pre-wrap`. Do NOT revert any of these to `whitespace-nowrap` or the default (which collapse runs to one space and break exact string-splitting).
30. **Bidi / RTL: extraction runs on LOGICAL order, the screen shows VISUAL order.** Narratives mix Arabic (RTL) with English/numbers (LTR). Extraction and position-based transformations operate on JS string indices (logical/stored order), but the browser renders mixed text in visual order, so operators can't tell where a split lands. [src/utils/bidi.ts](src/utils/bidi.ts) `containsRtl()` detects RTL by code point (numeric ranges, ASCII-only source — do NOT write literal RTL chars or a regex literal in that file; a literal once smuggled in U+FEFF and tripped `no-irregular-whitespace`). [src/components/shared/CharacterBreakdown.tsx](src/components/shared/CharacterBreakdown.tsx) renders text one char per cell in strict logical L→R order, each cell `dir="ltr"` + `unicode-bidi: isolate` so bidi can't reorder, with an index ruler and the captured span tinted; its `HighlightedText` export wraps a `<mark>` over the captured span on a `dir="auto"` container (windowed for long text). The extraction preview ([AttributeEditor.tsx](src/components/wizard/AttributeEditor.tsx), `extractionPreview` memo computes `captureStart/captureEnd` from `match.index` + the group's offset within `match[0]`, no `d`-flag) and the transformation preview ([TransformationPreview.tsx](src/components/wizard/TransformationPreview.tsx), only for position methods) show the breakdown gated by the SAME global "Character view" toggle as the table (threaded via a `characterView` prop: StepAttributes → AttributeEditor → TransformationList → TransformationPreview; inline builder passes `charViewEnabled`, the wizard reads persisted `tep:charView` once). Turning the toggle off reverts the rule builder previews to normal too — there is no separate per-preview toggle. Everything else (table narrative cells, View Context, AttributeDisplay, distinct lists, preview `<code>`) just gets `dir="auto"` for baseline per-value direction — a deliberately subtle change (no-op for Latin-first values; only flips alignment for Arabic-first ones). **Table "Character view" toggle:** a compact button in the filter row's `endSlot`, next to the Columns picker ([TransactionsTab.tsx](src/components/transactions/TransactionsTab.tsx), state `charViewEnabled`/`charViewCols`, persisted `tep:charView`/`tep:charViewCols`, default off / Additional Information) whose popover holds the on/off + a per-narrative-column checklist. When on, [TransactionTable.tsx](src/components/transactions/TransactionTable.tsx) renders RTL-containing cells in the selected columns via `SegmentedRtlText` (in CharacterBreakdown.tsx): it breaks ONLY the Arabic runs into per-char isolated cells and leaves LTR runs as normal flowing text, so the cell stays compact (exploding the whole value made cells huge). Rule/search match highlights are preserved in char-view: `computeHighlightRanges` (extracted from `highlightText`) computes the match spans and feeds `SegmentedRtlText`'s `highlightRanges`, which `<mark>`s matched LTR runs and tints matched Arabic cells — without it the highlighted prefix (e.g. `/ORDP`) vanished in char-view cells. The `charViewColumns` set is threaded through `RowCtx` + the `rowCtx` memo (gotcha #23) with a stable `EMPTY_CHAR_VIEW_COLUMNS` default. Display only; no extraction/transformation logic changed.
...
| Bidi / RTL (Arabic) text clarity              | [src/utils/bidi.ts](src/utils/bidi.ts), [src/components/shared/CharacterBreakdown.tsx](src/components/shared/CharacterBreakdown.tsx), [src/components/wizard/AttributeEditor.tsx](src/components/wizard/AttributeEditor.tsx), [src/components/wizard/TransformationPreview.tsx](src/components/wizard/TransformationPreview.tsx) (gotcha #30) |
...
- 2026-06-15 fix(transactions): hidden total/tally were inflated (UI showed 6002 with 0 hidden for a 5863-total, 27-hidden scope). Root cause: the backend KEEPS untagged rows under the `NI` exclusion (the "bd1267f trap" no longer applies), so the extra `OpsIsUntagged` half double-counted untagged rows (`5836 NI + 166 untagged = 6002`). Dropped the untagged half + `mergeSortedRows` from the live path — `replaceFromBeginningExcluding` now runs the NI visible query (its count IS the exact visible total) + a `PageSize 1` no-exclusion scope count written to `totalTransactionsCount`, so `totalHidden = totalTransactionsCount − visibleTotal` (5863 − 5836 = 27). Also removed the engine's separate `resolveUnfilteredTotal`/`fetchCount` count call. Gotcha #2 updated.
...
- 2026-06-15 feat(transformations): new `collapse_spaces` transformation ("Collapse Spaces") — collapses runs of 2+ literal spaces to one, leaving tabs/newlines and the ends untouched (distinct from `collapse_whitespace`). Mirrored in [src/utils/transformations.ts](src/utils/transformations.ts) + [src/constants/transformations.ts](src/constants/transformations.ts) for the preview; backend owns the `ATTRIBUTE_TRANSFORMATON` LOV entry + server-side apply (gotcha #28).
- 2026-06-15 feat(rule-builder): "See all distinct values" on a LOV-based extraction now shows the LOV item's English `Description` as the primary label (fallback: raw value), via a new `lovDescriptionLookup` in [src/context/LovAttributesContext.tsx](src/context/LovAttributesContext.tsx). Both the in-memory and backend ([DistinctValuesModal.tsx](src/components/wizard/DistinctValuesModal.tsx)) distinct lists use it; the modal prop is `descriptionMap`.
...
- 2026-06-15 fix(transactions): preserve consecutive spaces in narrative displays so on-screen text matches the raw data for exact string-splitting. `ViewContextModal` cells use `whitespace-pre`; `AttributeDisplay` and the wizard extraction/transformation preview `<code>` blocks use `whitespace-pre-wrap` (main table cells were already correct). Gotcha #29.
- 2026-06-16 feat(wizard): auto-scroll a freshly added condition / rule set / attribute into view on "+ Add…", via `useScrollNewItemIntoView` ([src/hooks/useScrollNewItemIntoView.ts](src/hooks/useScrollNewItemIntoView.ts)) applied in `RuleGroupEditor` (conditions), `StepRuleExpressions` + `MatchingRulesFilterButton` (rule sets), `StepAttributes` (attributes). Scrolls the list's last child with `block: 'nearest'` + a temporary `scroll-margin-bottom` so the trailing "+ Add…" button stays visible. Covers both the wizard and the inline Transactions-tab builder (shared components).
- 2026-06-15 feat(wizard): bidi/RTL clarity for splitting on Arabic narratives. New `containsRtl` ([bidi.ts](src/utils/bidi.ts)) + `CharacterBreakdown`/`HighlightedText` ([CharacterBreakdown.tsx](src/components/shared/CharacterBreakdown.tsx)). The extraction preview highlights the captured span on the real text and (auto when RTL) shows a logical-order letter-by-letter breakdown with indices; the transformation preview shows it for position-based transforms; table/modal/distinct-value renderers get `dir="auto"` for baseline per-value direction. Display only. Gotcha #30.
...
3. `npm run dev`, open the app, and walk the golden path for Transactions:
   ...
   3. Hide a tag in a row -> confirm the table auto-refills back to the previous visible row count (network panel: two parallel GetMT940Transactions — the NI-excluded visible query + a PageSize-1 no-exclusion scope count; no loop), header shows total showing + "N hidden" (= main-load total − visible total; e.g. 5863 total, hide 27 -> 5836 showing · 27 hidden), footer total matches the header and stays stable (no counting down).
